 ...
 *
 * Most of bitops implementation adopted from OpenSBI project
 *
 * Authors:
...
#define for_each_set_bit(bit, word, tmp) \
    for (bit = sbi_ffs(word), tmp = word; tmp; tmp = word >> (bit + 1) , bit += (sbi_ffs(tmp) + 1))
